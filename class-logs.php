...
public static function get_logs( $per_page = 10, $page_number = 1, $email = '' ) {
   ...
   //  //$url = 'https://api.pepipost.com/v2/logs';
...
curl_setopt_array($curl, array(
  CURLOPT_URL => "https://api.pepipost.com/v2/logs?sort=desc&startdate=2018-10-01&limit=1000",
    ...
    if( !empty($records) ) {
        ...
        foreach($records as $rec) {
            ...
            foreach ( $columns as $column_name => $column_display_name ) {
                ...
                //Display the cell
                switch ( $column_name ) {
                    case "adate":  echo '<td '.$attributes.'>'.stripslashes($rec['requestedTime']).'</td>';   break;
                    case "fromaddress": echo '<td '.$attributes.'>'.stripslashes($rec['fromaddress']).'</td>'; break;
                    case "email": echo '<td '.$attributes.'>'.stripslashes($rec['rcptEmail']).'</td>'; break;
                    case "msize": echo '<td '.$attributes.'>'.$rec['size'].'</td>'; break;
                    case "arrivaltime": echo '<td '.$attributes.'>'.$rec['deliveryTime'].'</td>'; break;
...
public static function get_stats( $start = '', $end = '' ) {
    ...
    // $result = array();
    // //$url = 'https://api.pepipost.com/v2/stats';
    // $url = 'https://api.pepipost.com/v2/stats';
    // $data = array( 
    //     'start_date' => $start, 
    //     'end_date'   => $end, 
...
curl_setopt_array($curl, array(
  CURLOPT_URL => "https://api.pepipost.com/v2/stats?startdate=$start&enddate=$end",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 30,
