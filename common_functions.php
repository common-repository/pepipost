...
function myfunc()
{
    ...
    $curl = curl_init();
	      ...
	      curl_setopt_array($curl, array(
	        CURLOPT_URL => "https://api.pepipost.com/v2/logs?enddate=".$today_data."&startdate=2018-01-01&limit=10",
...
function wp_pepipost_options_page() {
	...
	if (isset($_POST['wpp_action']) && $_POST['wpp_action'] == __('Send Test', 'wp_pepipost') && isset($_POST['to'])) {
		...
		$to = ltrim($to, ",");
		if ( !filter_var($to, FILTER_VALIDATE_EMAIL) && !filter_var($reply, FILTER_VALIDATE_EMAIL)) { ?>
		     <div id="pepi_login_error" class="error"><p>Please enter valid email address.</p></div>
			 <?php 
		...
		<?php if ( $result ) { 
			 ...
			 echo '<div id="pepi_message" class="updated fade"><p><strong>' . $test . '</strong></p></div>'; 
		?>

  <!-- <div id="pepi_message" class="updated"><p><strong><?php// _e('Test Message Sent', 'wp_pepipost'); ?></strong></p> -->

</div>
<?php } else { 
        
?>
<div id="pepi_login_error" class="error fade"><p><strong><?php _e('Error while sending test message.', 'wp_pepipost'); ?></strong></p>
    <?php if ( !empty($error) ) {
...
<?php } ?>
	...
	if( isset($_REQUEST['wp_pepipost_option']) && $_REQUEST['wp_pepipost_option'] == 1 ) {
		...
		?>
		<div id="pepi_message" class="updated"><p><strong><?php _e('Settings saved successfully', 'wp_pepipost'); ?></strong></p></div>
<?php
	}
...
<div class="w-logo  with_transparent">
	...
	<div id="pepi_smtp_error" class="dspln"></div>
</div> <!-- end w-logo -->
...
<td><input name="api_key" type="text" id="pepi_api_key" value="<?php print(get_option('wpp_api_key')); ?>" size="40" class="regular-text" required/>
...
<td><input name="mail_from_name" type="text" id="pepi_mail_from_name" value="<?php echo (isset($frm_name)) ? $frm_name: ''// print(get_option('wpp_mail_from_name'));//  ?>" size="40" class="regular-text"  />
...
<td><input name="mail_from" type="text" id="pepi_mail_from" value="<?php print(get_option('wpp_mail_from')); ?>" size="40" class="regular-text" />
...
<td><input name="mail_reply" type="text" id="pepi_mail_reply" value="<?php echo (isset($frm_reply)) ? $frm_reply: ''//print(get_option('wpp_mail_reply')); ?>" size="40" class="regular-text" autocomplete="off" />
...
<p class="submit"><input type="submit" name="submit" id="pepi_submit-pepipost" class="button-primary" value="<?php _e('Save Changes'); ?>" /></p>
</p>
...
<div id="pepi_show_error" class="dspln"></div>
...
<td><input name="to" type="email" id="pepi_to" value="" size="40" class="code" required="true" />
<span class="description"><?php _e('Type your email address here.', 'wp_pepipost'); ?></span></td>
</tr>

<tr valign="top">
<th scope="row"><label for="to">Subject</label></th>
<td><input name="subject" type="text" id="pepi_subject" value="" size="40" class="code" required="true"/>
<span class="description">Type your email subject here.</span></td>
</tr
<tr valign="top">
<th scope="row"><label for="to">Message</label></th>
<td><textarea name="message" id="pepi_email_message" class="code" required="true"></textarea>
...
<table id="pepi_example" class="display" style="width:100%">
        <thead>
            <tr>
                <th>Date</th>
                <th>From Address</th>
...
window.onload = function () {
	var chart = new CanvasJS.Chart("chartContainer",
	{
		...
		data: [
		{
			...
			dataPoints: [
		{  y: <?php echo $today_data[0]['stats'][0]['metrics']['sent']; ?>, label: "Delivered" },
        {  y: <?php echo $today_data[0]['stats'][0]['metrics']['bounce']; ?>, label: "Bounced"},
        {  y: <?php echo $today_data[0]['stats'][0]['metrics']['open']; ?> , label: "Opened"},
        {  y: <?php echo $today_data[0]['stats'][0]['metrics']['click']; ?>, label: "Clicked" },
        {  y: <?php echo $today_data[0]['stats'][0]['metrics']['dropped']; ?>, label: "Dropped" },
        {  y: <?php echo $today_data[0]['stats'][0]['metrics']['invalid']; ?>, label: "Invalid"},
        {  y: <?php echo $today_data[0]['stats'][0]['metrics']['unsub']; ?>, label: "Unsubscribes"},
        {  y: <?php echo $today_data[0]['stats'][0]['metrics']['spam']; ?>, label: "Spam"}
	...
	var chart = new CanvasJS.Chart("chartContainerSec",
	{
        ...
        {  y: <?php echo $monthly_unsub; ?>, label: "Unsubscribes"},
        {  y: <?php echo $monthly_spam; ?>, label: "Spam"}
...
<script>
    jQuery(document).ready(function() {
        jQuery('#pepi_from_date').val( "<?php echo date( 'm/d/Y', strtotime($start_date) ); ?>" );
        jQuery('#pepi_to_date').val( "<?php echo date( 'm/d/Y',strtotime($end_date) ); ?>" );
    });
</script>
<?php } ?>
<div id="dateRangeDiv"><h4>From <?php echo date('d M, Y', strtotime($start_date)); ?> to <?php echo date('d M, Y', strtotime($end_date)); ?></h4>
<form id="pepi_dateRangeForm" method="post" action="admin.php?page=wp_pepipost_stats">
<?php wp_nonce_field('wpp_from_to', 'wpp_from_to_field'); ?>
<label for="from">From</label>
<input type="text" id="pepi_from_date" name="wpp_from_date" required >
<label for="to">to</label>
<input type="text" id="pepi_to_date" name="wpp_to_date" required>

<!-- Date range picker 
<div id="pepi_reportrange" style="background: #fff; cursor: pointer; padding: 10px 0px; border: 1px solid #ccc; width: 30%; z-index: 99999; position: absolute">
...
/*$(function() {
    ...
    function cb(start, end) {
        $('#pepi_reportrange span').html(start.format('MMMM D, YYYY') + ' - ' + end.format('MMMM D, YYYY'));
    }

    $('#pepi_reportrange').daterangepicker({
        startDate: start,
        endDate: end,
